...
from recipes.models import (
    Ingredient, IngredientInRecipe, Recipe, Tag, TagInRecipe
)
...
class CustomUserSerializer(UserSerializer):
    ...
    def get_is_subscribed(self, obj):
        ...
        if (user.id is None) or (user.id == obj.id):
            return False
...
class RecipeSerializer(serializers.ModelSerializer):
    ...
    def get_is_in_shopping_cart(self, obj):
        ...
        if request is None or request.user.is_anonymous:
            return False
        return obj.shopping_recipe.filter(user=request.user).exists()
...
# Как я понял все работало, после первого ревью я внес изменения, но не
# пересобрал образ т.к. образы собираю через виртуальную машину, а это очень
# долго, решил схитрить. После того как пересобрал образ валидация работает.
# Скриншот с работающей валидацией на сайте,в виде фотографии рецепта.
# Не знаю почему, но на удаленном сервере переодичскеи слетает БД.
# Из-за чего списки рецептов исчезают,а так держу 2 наполненые страницы.
# Приходиться выполнять миграции и по новой выкладывать список рецептов.
...
class CreateRecipeSerializer(serializers.ModelSerializer):
    ...
    def validate_ingredients(self, data):
        ...
        for element in ingredients:
            ...
            lst_ingredient.add(ingredient_id)
    ...
    def create_ingredients(self, ingredients, recipe):
        ...
        for _ in ingredients:
            ...
            amount = _['amount']
            bulk_list.append(IngredientInRecipe(
